## Task: Bulk Judge CSV Import Component

**Context:**
- File: src/components/BulkJudgeImport.tsx
- Pattern: Follow existing BulkStudioImportModal.tsx pattern
- Usage: Competition Director adds multiple judges at once

**Requirements:**
1. Modal with file upload input
2. CSV format: firstName, lastName, email, phone, certification (optional)
3. Preview table showing parsed data
4. Validation:
   - Email format check
   - Duplicate email check
   - Required fields (firstName, lastName, email)
5. Row-by-row error display
6. Import button (calls backend mutation)

**Deliverables:**
- Complete BulkJudgeImport.tsx component
- Export BulkJudgeImportModal
- CSV parsing logic included
...
export function BulkJudgeImportModal({ isOpen, onClose, onImportComplete }: BulkJudgeImportModalProps) {
...
**CSV Example:**
```csv
firstName,lastName,email,phone,certification
John,Doe,john@example.com,555-1234,Level 3
Jane,Smith,jane@example.com,555-5678,Level 2
```
...
- certification: Optional, free text

**Codex will**: Generate complete bulk import component
**Claude will**: Create backend mutation (judge.bulkImport), integrate modal
