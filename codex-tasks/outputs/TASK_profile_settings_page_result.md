# Result: Competition Director Profile Settings Page

- File added: `src/app/dashboard/settings/page.tsx`
- File added: `src/components/ProfileSettingsForm.tsx`
- Backend: `src/server/routers/user.ts` already contains `updateProfile` and `getCurrentUser`
...
- Uses glassmorphic styling and gradient title
- RHF + Zod for form validation
- tRPC `getCurrentUser` and `updateProfile` used
- Toast notifications for success/error
- Loading state on submit button

## Router Registration
`src/server/routers/user.ts` is already registered in `src/server/routers/_app.ts` as `user`.
