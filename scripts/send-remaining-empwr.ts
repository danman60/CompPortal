/**
 * Send remaining EMPWR emails (Elite Star + Fever only)
 * Academy of Dance Arts and Cassiahs already sent
 */
...
// ONLY Elite Star and Fever - others already sent
const REMAINING_STUDIOS = [
  {
    name: 'Elite Star',
...
function generateEmail(studio: typeof REMAINING_STUDIOS[0]): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
async function sendRemaining() {
  console.log('=== Sending Remaining EMPWR Emails ===\n');
  console.log('(Academy of Dance Arts and Cassiahs already sent)\n');

  for (const studio of REMAINING_STUDIOS) {
