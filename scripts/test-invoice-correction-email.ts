/**
 * Test script to send invoice correction email sample
 * Run with: npx tsx scripts/test-invoice-correction-email.ts
 */
...
// Sample data for Academy of Dance Arts (EMPWR) - $1,320 Production fee correction
const sampleStudio = {
  ...
  originalInvoiceTotal: 10756.47,
  amountPaid: 0.00,
  correctedTotal: 12098.91,
  balanceDue: 11598.91,  // After $500 deposit
  // Invoice URL components
  subdomain: 'empwr',
  studioId: '9f96460e-904c-4d12-8f50-aaeae27367df',
  competitionId: '05c0eae4-cb2f-44cc-9c5e-6b2eed700904',
  branding: {
    primaryColor: '#FF1493',  // EMPWR pink
    name: 'EMPWR Dance Experience',
  }
};

function generateInvoiceCorrectionEmail(studio: typeof sampleStudio): string {
  return `
<!DOCTYPE html>
<html>
<head>
    ...
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <!-- Header -->
          <tr>
            <td style="background: linear-gradient(135deg, ${studio.branding.primaryColor} 0%, #9333ea 100%); padding: 30px 40px; text-align: center;">
              <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: bold;">${studio.branding.name}</h1>
          ...
          <tr>
            <td style="padding: 40px;">
              ...
              <div style="background-color: #f9fafb; border-radius: 8px; padding: 24px; margin: 24px 0;">
                ...
                <table width="100%" cellpadding="0" cellspacing="0" style="font-size: 14px;">
                  <tr>
                    ...
                    <td style="color: #374151; text-align: right; padding: 8px 0;">$${studio.originalInvoiceTotal.toFixed(2)}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td style="color: ${studio.branding.primaryColor}; text-align: right; padding: 8px 0; font-weight: 700; font-size: 18px;">$${studio.balanceDue.toFixed(2)}</td>
                  </tr>
                </table>
              </div>
              ...
              <div style="text-align: center; margin: 32px 0;">
                <a href="https://${studio.subdomain}.compsync.net/dashboard/invoices/${studio.studioId}/${studio.competitionId}" style="display: inline-block; background: linear-gradient(135deg, ${studio.branding.primaryColor} 0%, #9333ea 100%); color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">View Updated Invoice</a>
              </div>
              ...
              <p style="color: #374151; font-size: 16px; line-height: 1.6; margin: 24px 0 0 0;">
                ...
                <strong>The ${studio.branding.name} Team</strong>
              </p>
            </td>
          </tr>
...
async function sendTestEmail() {
  console.log('Sending test invoice correction email...\n');
  console.log(`Studio: ${sampleStudio.name}`);
  console.log(`Balance Due: $${sampleStudio.balanceDue.toFixed(2)}`);
  console.log(`Sending to: ${RECIPIENT}\n`);

  const html = generateInvoiceCorrectionEmail(sampleStudio);

  const result = await sendEmail({
    to: RECIPIENT,
    subject: `[TEST] Invoice Correction - ${sampleStudio.competition} - ${sampleStudio.name}`,
    html,
  ...
  if (result.success) {
    ...
    console.log('\nCheck your inbox at danieljohnabrahamson@gmail.com');
  } else {
