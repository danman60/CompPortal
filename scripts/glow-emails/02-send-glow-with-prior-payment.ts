/**
 * Send Glow correction emails - Studios WITH prior payments
 * These studios only owe the DIFFERENCE (Production fee)
 *
 * Run with: npx tsx scripts/glow-emails/02-send-glow-with-prior-payment.ts
 *
...
// Studios with prior payments - only owe the difference
// IDs verified from database query 2025-12-23
const STUDIOS_WITH_PAYMENT = [
  {
    ...
    participants: 41,
    productionFee: 2255.00, // 41 × $55
    correctedTotal: 13622.15,
    amountPaid: 5798.31,
    balanceDue: 2323.84,
    subdomain: 'glow',
    studioId: 'e3d4b5ad-8189-4260-82fb-716b9d29d860',
    competitionId: '5607b8e5-06dd-4d14-99f6-dfa335df82d3',
  },
  {
    name: 'Dancecore',
    email: 'info@dancecoreinc.com',
    competition: 'GLOW St. Catharines Spring 2026',
    entryTitle: 'Conga',
    participants: 36,
    productionFee: 1980.00, // 36 × $55
    correctedTotal: 15492.30,
    amountPaid: 12754.90,
    balanceDue: 2237.40,
    subdomain: 'glow',
    studioId: 'd481685c-b9f2-489e-aa51-29f1f3b4f15f',
  ...
  },
  {
    ...
    correctedTotal: 37081.52,
    amountPaid: 33327.11,
    balanceDue: 3254.41,
    subdomain: 'glow',
    studioId: '3c973ea4-da21-48d3-bef4-e116a7a02bc7',
    competitionId: '5607b8e5-06dd-4d14-99f6-dfa335df82d3',
  },
];

function generateEmail(studio: typeof STUDIOS_WITH_PAYMENT[0]): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
async function sendEmails() {
  console.log('=== Glow Correction Emails (WITH Prior Payment) ===\n');
  console.log('These studios only owe the DIFFERENCE.');
  console.log('Rate limit: 2 seconds between emails\n');

  for (let i = 0; i < STUDIOS_WITH_PAYMENT.length; i++) {
    const studio = STUDIOS_WITH_PAYMENT[i];
    console.log(`[${i + 1}/${STUDIOS_WITH_PAYMENT.length}] ${studio.name}: ${studio.email}`);
    ...
    if (i < STUDIOS_WITH_PAYMENT.length - 1) {
