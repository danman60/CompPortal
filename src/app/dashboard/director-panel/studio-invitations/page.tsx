...
/**
 * Studio Invitations Management (CD Version)
 * Competition Director - Tenant-scoped studio invitation and onboarding tracking
 * Features: Sort by event date, studio name, invited status, claimed status, onboarding status
 */
...
type SortField = 'name' | 'eventDate' | 'invited' | 'claimed' | 'onboarding';
type SortDirection = 'asc' | 'desc';
type FilterStatus = 'all' | 'unclaimed' | 'claimed' | 'invited' | 'not-invited' | 'onboarding-complete' | 'onboarding-pending';

export default function CDStudioInvitationsPage() {
  ...
  const { data, refetch, isLoading} = trpc.studioInvitations.getStudiosForCD.useQuery();

  // Fetch competitions for CD's tenant
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-indigo-900 to-blue-900 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-300">Manage studio invitations and onboarding for your competition</p>
            </div>
            <a
              href="/dashboard"
                        ...
                        {editingStudioId === studio.id ? (
                          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 mb-2">
                            <input
                              ...
                              autoFocus
                              className="w-full sm:w-auto flex-1 px-3 py-2 min-h-[44px] bg-white/10 border border-purple-400 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                            />
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <button
                              ...
                              className="min-h-[44px] min-w-[44px] px-2 py-1 bg-white/10 hover:bg-white/20 border border-white/20 rounded text-gray-300 hover:text-white text-xs transition-colors"
                              title="Edit email"
                            >
                              ✏️
                            </button>
        ...
        {/* Quick Add Studio Modal (CD Version - No Tenant Selector) */}
        {addStudioModal && (
