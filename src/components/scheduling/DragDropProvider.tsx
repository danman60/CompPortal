...
export function DragDropProvider({
  ...
  const handleBlockDrag = (draggedBlockId: string, targetId: string) => {
    ...
    if (targetId.startsWith('block-')) {
      ...
      console.log('[DragDropProvider] Moving', movingDown ? 'DOWN' : 'UP');
      ...
      // Reorder logic:
      // Moving DOWN: After removing fromIndex, toIndex shifts down by 1, so use toIndex - 1
      // Moving UP: After removing fromIndex, toIndex stays same, so use toIndex directly
      const reordered = [...sortedBlocks];
      const [removed] = reordered.splice(fromIndex, 1);

      const insertIndex = movingDown ? Math.max(0, toIndex - 1) : toIndex;

      console.log('[DragDropProvider] Inserting at index:', insertIndex, '(adjusted from toIndex:', toIndex, 'direction:', movingDown ? 'DOWN' : 'UP', ')');
    ...
    // If dropped on routine, insert before that routine
      ...
      console.log('[DragDropProvider] Block-to-Routine drag: Using index-based reordering (Attempt 12 with proper index adjustment)');
      ...
      // CRITICAL FIX: Adjust insert index based on drag direction (matches routine logic)
      const movingDown = draggedIndex < targetIndex;
      const reordered = [...timeline];
      const [removed] = reordered.splice(draggedIndex, 1);
      // When moving DOWN, after removing item at draggedIndex, all indices shift down by 1
      // So use targetIndex - 1. When moving UP, use targetIndex directly.
      const insertIndex = movingDown ? Math.max(0, targetIndex - 1) : targetIndex;
      reordered.splice(insertIndex, 0, removed);
