...
export const schedulingRouter = router({
  ...
  // Get Trophy Helper data: last routine per overall category
  getTrophyHelper: publicProcedure
    .input(z.object({ competitionId: z.string().uuid() }))
    .query(async ({ input }) => {
      // Get all scheduled entries with necessary joins
      const entries = await prisma.competition_entries.findMany({
        where: {
          competition_id: input.competitionId,
          session_id: { not: null }, // Only scheduled entries
      ...
      // Group by overall category (Size + Age + Classification)
      const categoryMap = new Map<string, {
        categoryName: string;
        lastRoutineId: string;
        lastRoutineNumber: number | null;
        lastRoutineTime: Date;
        totalCount: number;
        suggestedAwardTime: Date;
        entrySizeCategory: string;
        ageGroup: string;
        classification: string;
      }>();

      for (const entry of entries) {
        const categoryKey = `${entry.entry_size_categories.name}-${entry.age_groups.name}-${entry.classifications.name}`;
        const categoryName = `${entry.entry_size_categories.name} - ${entry.age_groups.name} - ${entry.classifications.name}`;

        const existing = categoryMap.get(categoryKey);

        // Skip entries without performance time (already filtered in where clause, but type safety)
        if (!entry.performance_time) continue;

        if (!existing) {
          // First entry in this category
          const suggestedAwardTime = new Date(entry.performance_time);
          const routineDuration = entry.routine_length_minutes || 3; // Default 3 minutes if not set
          suggestedAwardTime.setMinutes(suggestedAwardTime.getMinutes() + routineDuration + 30); // +duration +30 min

          categoryMap.set(categoryKey, {
            categoryName,
            lastRoutineId: entry.id,
            lastRoutineNumber: entry.entry_number,
            lastRoutineTime: entry.performance_time,
            totalCount: 1,
            suggestedAwardTime,
            entrySizeCategory: entry.entry_size_categories.name,
            ageGroup: entry.age_groups.name,
            classification: entry.classifications.name,
          });
        } else {
          // Update if this is a later routine
          const currentTime = new Date(entry.performance_time);
          const existingTime = new Date(existing.lastRoutineTime);

          if (currentTime > existingTime) {
            const suggestedAwardTime = new Date(entry.performance_time);
            const routineDuration = entry.routine_length_minutes || 3; // Default 3 minutes if not set
            suggestedAwardTime.setMinutes(suggestedAwardTime.getMinutes() + routineDuration + 30);
      ...
      // Convert map to array and sort by suggested award time
      const trophyHelper = Array.from(categoryMap.values()).sort((a, b) =>
        a.suggestedAwardTime.getTime() - b.suggestedAwardTime.getTime()
      );
